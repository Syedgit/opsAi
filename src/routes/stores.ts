...
storesRouter.get('/mock', (_req, res) => {
  res.json({
    stores: [
      {
        id: '1',
...
// Mock dashboard endpoint - returns different stats per store
storesRouter.get('/mock-dashboard', (req, res) => {
...
// Mock data endpoint - returns different data per store
storesRouter.get('/mock-data', (req, res) => {
  const storeId = (req.query.storeId as string) || 'S001';
...
// Mock detail endpoint - returns detailed info for a specific record
storesRouter.get('/mock-detail', (req, res) => {
  const storeId = (req.query.storeId as string) || 'S001';
  const type = (req.query.type as string) || 'sales';
