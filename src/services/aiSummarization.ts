...
Return JSON format:
{
  ...
  "recommendations": ["recommendation 1", "recommendation 2", "recommendation 3"]
    ...
    try {
      ...
      if (errorStatus === 429) {
        logger.debug('OpenAI API quota exceeded, using fallback insights', { 
          storeId,
          period 
        });
      } else {
        logger.warn('OpenAI API error, using fallback insights', { 
          error: errorMessage, 
          status: errorStatus,
          storeId,
          period 
    ...
    logger.error('Failed to generate business summary', {
      error: error.message,
      storeId,
      period,
    });
...
export async function generateWhatsAppSummary(
  ...
  try {
    ...
    logger.error('Failed to generate WhatsApp summary', {
      error: error.message,
      storeId,
      period,
    });
